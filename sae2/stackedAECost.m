...
y_h1 = sigmoid(stack{1}.w * data + repmat(stack{1}.b,1,size(data,2)));
y_h2 = sigmoid(stack{2}.w * y_h1 + repmat(stack{2}.b,1,size(data,2)));
M = softmaxTheta * y_h2;
...
softmaxThetaGrad = -dataMean * (groundTruth - P) * y_h2' + lambda  * softmaxTheta;
cigma3 = -softmaxTheta' * (groundTruth - P) .* y_h2 .* (1 - y_h2);
